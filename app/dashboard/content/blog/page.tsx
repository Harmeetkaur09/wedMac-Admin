  ...
  const createOrUpdate = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          "Please fill required fields: title, content, hashtags, category."
        );
      }
      ...
      if (editingId && photosToDelete.size > 0) {
        form.append(
          "images_to_delete",
          JSON.stringify(Array.from(photosToDelete))
        );
      }
      ...
      if (!editingId) {
        // ✅ CREATE
        if (!projectId.trim())
          throw new Error("project_id is required for create.");
    ...
    <div className="space-y-6 animate-fade-in">
        ...
        <CardContent>
          <form onSubmit={createOrUpdate} className="space-y-4 pt-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <Input
                placeholder="Project ID (required for create)"
                value={projectId}
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-3">
                  {existingPhotos.map((url) => (
                    <div
                      key={url}
                      className="relative w-24 h-24 bg-gray-100 rounded overflow-hidden"
                    >
                      <img
                        src={url}
                        alt="photo"
                      ...
                      <label className="absolute top-1 right-1 bg-white rounded p-1 text-xs cursor-pointer">
                        <input
                          type="checkbox"
                          checked={photosToDelete.has(url)}
                          onChange={() => onTogglePhotoDelete(url)}
                        />{" "}
                        Delete
                      </label>
                    </div>
      ...
      <Tabs defaultValue="all" className="w-full">
                          ...
                          <div className="flex flex-col md:flex-row gap-6">
                            ...
                            <div className="flex-1 space-y-3">
                              <div className="flex justify-between items-start">
                                <div>
                                  <h3 className="text-xl font-bold hover:text-[#FF6B9D] cursor-pointer">
                                    {post.title}
                                  </h3>
                              ...
                              <div className="flex gap-2 pt-2">
                                ...
                                <Button
                                  ...
                                  onClick={() =>
                                    window.open(
                                      `https://wed-mac-qsxz.vercel.app/blog/${
                                        post.project_id || post.id
                                      }`,
                                      "_blank"
                                    )
                                  }
        ...
        <TabsContent value="published">
          <Card>
            <CardContent className="p-6">
              Published blog posts will be displayed here.
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="draft">
          <Card>
            <CardContent className="p-6">
              Draft blog posts will be displayed here.
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="scheduled">
          <Card>
            <CardContent className="p-6">
              Scheduled blog posts will be displayed here.
            </CardContent>
          </Card>
        </TabsContent>
      </Tabs>
