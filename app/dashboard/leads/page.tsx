...
const API_URL = "https://wedmac-be.onrender.com/api/leads/list/";
...
const mapToLead = (raw: RawLead): Lead => {
  ...
  // build a readable name: prefer first_name + last_name, else fallback to other fields
  let name = "Unknown";
  if ((raw.first_name && String(raw.first_name).trim()) || (raw.last_name && String(raw.last_name).trim())) {
  ...
  const phone = raw.phone ?? raw.contact_no ?? raw.mobile ?? raw.client_phone ?? null;

  // service: backend returns numeric id (service), but event_type exists in your payload -> use event_type as readable fallback
  const service = raw.service && typeof raw.service !== "object" ? String(raw.service) : raw.event_type ?? "";

  // location may be an id; try to use textual fields if present
  const location = raw.location_name ?? raw.location_text ?? (raw.location ? String(raw.location) : "");
...
export default function LeadManagement() {
  ...
  const fetchLeads = async () => {
    ...
    try {
      ...
      // 1) array at root
      // 2) { results: [...] } or { data: [...] }
      // 3) { leads: [...] }  <-- your backend
      const items = Array.isArray(data) ? data : data.leads ?? data.results ?? data.data ?? [];
  ...
  const filteredLeads = useMemo(() => {
    const byTab = activeTab === "all" ? leads : leads.filter((l) => l.status === activeTab);
                    ...
                    <TableBody>
                      {filteredLeads.map((lead) => (
                        <TableRow key={String(lead.id)}>
                          ...
                          <TableCell>{lead.location ?? String(lead.raw?.location ?? "-")}</TableCell>
                          <TableCell>
