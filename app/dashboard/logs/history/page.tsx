...
export default function ArtistActivityLogsByArtistPage() {
  ...
  // meta from per-artist API
  const [artistIdFromResp, setArtistIdFromResp] = useState<number | null>(null);
  const [artistNameFromResp, setArtistNameFromResp] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    async function fetchArtists() {
      try {
        ...
        const res = await fetch("https://api.wedmacindia.com/api/admin/artists/?Status=all", {
  ...
  async function fetchArtistLogs(artistId: number, page = 1) {
    setLoading(true);
    setError(null);
    try {
      const token = getToken();
      const url = `https://api.wedmacindia.com/api/artists/admin/${artistId}/activity-logs/?page=${page}&page_size=${pageSize}`;
      ...
      // expected shape: { artist_id, artist_name, logs: [...], total_count, total_pages, current_page, page_size }
      setRawJson(data);
      setArtistIdFromResp(data.artist_id ?? null);
      setArtistNameFromResp(data.artist_name ?? null);
      ...
      console.error("fetchArtistLogs:", err);
      setError(err?.message || "Failed to fetch logs");
      setLogs([]);
      setArtistIdFromResp(null);
      setArtistNameFromResp(null);
      setTotalCount(0);
      setTotalPages(1);
      setCurrentPage(1);
    } finally {
  ...
  // when selectedArtist changes, fetch logs for that artist
  useEffect(() => {
    ...
    fetchArtistLogs(selectedArtist, 1);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedArtist]);

  // page change fetch
  useEffect(() => {
    if (selectedArtist != null) fetchArtistLogs(selectedArtist, currentPage);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentPage]);
  ...
  function downloadRawJson() {
    ...
    a.href = url;
    a.download = `artist-${artistIdFromResp ?? selectedArtist}-activity-logs-${new Date().toISOString()}.json`;
    a.click();
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <Card className="mb-6">
        <CardHeader>
          <CardTitle>Artist Activity Logs (Per Artist)</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="bg-blue-50 p-4 rounded-lg">
              <p className="text-sm text-blue-600 font-medium">Selected Artist</p>
              <p className="text-2xl font-bold">{artistNameFromResp ?? "—"}</p>
              <p className="text-xs text-gray-500">ID: {artistIdFromResp ?? "—"}</p>
            </div>

            <div className="bg-green-50 p-4 rounded-lg">
              <p className="text-sm text-green-600 font-medium">Total Logs</p>
          ...
          {rawOpen && (
            <div className="mb-4">
              <div className="flex items-center justify-between mb-2">
                <div className="text-sm text-gray-600">Raw API response (per-artist)</div>
